...
def load_data():
    # Модуль binanceloader
    return None  # Поки що просто повертаємо None
...
def test_load_data_returns_data():
    # Перевірка, чи повертається дані
    data = load_data()
    assert data is not None

def test_load_data_returns_dataframe():
    # Перевірка, чи повертається DataFrame
    data = load_data()
    assert isinstance(data, pd.DataFrame)

def test_load_data_correct_data():
    # Перевірка, чи завантажується правильна кількість даних
    expected_num_rows = 100  # Припустимо, що очікуємо 100 рядків даних
    data = load_data()
    assert len(data) == expected_num_rows
